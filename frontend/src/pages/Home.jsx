...
function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/linktree/public/root');
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    const trackView = async () => {
      try {
        await fetch('/api/analytics/view/root', { method: 'POST' });
      } catch {
  ...
  const trackClick = async (platformType) => {
    try {
      await fetch('/api/analytics/click/root', {
        method: 'POST',
  ...
  const handleSponsorClick = () => {
    if (data?.sponsorPhone) {
      window.open(`https://wa.me/${formatIraqiPhone(data.sponsorPhone)}`, '_blank');
    }
  };
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/30">
        <div className="w-20 h-20 rounded-3xl bg-slate-100 flex items-center justify-center mb-4">
          <FaGlobe className="w-10 h-10 text-slate-300" />
        </div>
        <p className="text-slate-500 font-medium">پەڕە نەدۆزرایەوە</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/30 flex flex-col items-center px-5 py-10 sm:px-6 sm:py-12 md:px-8 md:py-14 lg:py-16 overflow-y-auto relative">
      ...
      <div className="relative z-10 flex flex-col items-center w-full">
        ...
        <div className="mt-14 sm:mt-16 flex flex-col items-center gap-4 flex-shrink-0">
          <p className="text-slate-400 text-lg sm:text-lg md:text-xl font-medium">{data.footerText}</p>
          <button
